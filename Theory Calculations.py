...
wavelength = 800e-9  # excitation photon wavelength

def excited_carriers_fraction(wavelength, deltaT):
    photon_energy_mol= h*c*mol/(wavelength)
    fraction = Cl*deltaT/(5*photon_energy_mol)
    return fraction

print(excited_carriers_fraction(wavelength, 99.2))
print(excited_carriers_fraction(wavelength, 221.7))
print(excited_carriers_fraction(wavelength, 378.6))
...
def DB_temperature(slope, room_temperature, verbose):
    ...
    idx, B_nearest = find_nearest(B_highT, BT_eq)   # finds the nearest temperature that matches BT_eq
    delta_T = T_high[idx] - room_temperature   # finds the temperature difference

    if verbose:
        print("==========================================")
        print("BT_eq = ", str(BT_eq))
        print("Closest B: ", B_nearest)
        print("Closest temperature: ", T_high[idx])
        print("delta T: ", delta_T)
        print(" ")

    return B_nearest, T_high[idx], delta_T

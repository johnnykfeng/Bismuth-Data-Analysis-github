...
for scan_index in np.arange(1,6,1):  # loop over scans
    ...
    for peak_index in peak_choice:  # loop over peaks
        ...
        #region Trying to subtract Liquid Rise trace from Peak trace
        ...
        # individual cases
        if scan_index == 1 and peak_index == 6:
            a, tau, t0, c = -0.1, 0.5, 1.7, 1
        if scan_index == 2 and peak_index == 3:
            a, tau, t0, c = 0.06, 0.04, 0.4, 1
            print(a, tau, t0, c)
        if scan_index == 2 and peak_index == 6:
            a, tau, t0, c = -0.2, 10, -1.2, 1
        if scan_index == 5 and peak_index == 3:
            a, tau, t0, c = 0.06, 0.04, 0.4, 1
        ...
        # adjusted the t0
        ax1.plot(tp- fit_var[2], peak_norm, '-o', label=peak_cols[peak_index],
                 color=peak_colors[peak_index], linewidth=0.4)

        ax1.plot(x_fit - fit_var[2], y_fit, '--', color = peak_colors[peak_index])
        #endregion

    fluence = scandict_df[scandict_df['scan_id'] == scan_index]['fluence'].values
    fig_str_label = 'scankey: ' + str(scan_index) + ', fluence: ' +str(fluence)
    fig1.suptitle(fig_str_label)
    plt.legend()
    plt.grid(True)
